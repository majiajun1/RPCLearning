...
import version4.part1.Client.cache.serviceCache;
import version4.part1.Client.serviceCenter.ZkWatcher.watchZK;
import version4.part1.Client.serviceCenter.balance.impl.ConsistencyHashBalance;
...
public class ZKServiceCenter implements ServiceCenter{
    ...
    public InetSocketAddress serviceDiscovery(String serviceName) {
        try {
            ...
            if(serviceList==null){
                serviceList=client.getChildren().forPath("/" + serviceName);//缓存没有 去服务端取
            }

//            List<String> strings = client.getChildren().forPath("/" + serviceName);//树结构管理服务

            // 这里默认用的第一个，后面加负载均衡
//            String string = serviceList.get(0);//存的是ip和端口 就是接口的地址啦
            String address=new ConsistencyHashBalance().balance(serviceList);
            return parseAddress(address); //解析
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
